...
class CCPrompts:
    ...
    def cc_history_prompt(texts):
        """CC 병력 및 습관 관련 프롬프트"""
        return f"""
            당신은 구강내과 전문의입니다. 아래는 환자의 상담 데이터의 주호소(CC) 중 환자의 턱관절 병력 및 습관 텍스트입니다.
            텍스트에 없는 정보는 절대 추가하지 말고, 명확하게 언급된 정보만 사용하세요.
            누락된 내용은 빈 문자열("") 또는 null로 처리하세요.
            아래 각 항목은 텍스트에 명시된 내용을 기반으로 정확히 추출해야 합니다.

            각 텍스트에 대해 다음 정보를 JSON 객체 형식으로 추출하세요:    
            다음 텍스트를 분석하여 JSON 형식으로 분류해주세요.
            텍스트에 없는 내용은 지어내지 마세요.
            
            1. dentalHistory_desc: 과거 치과적 개입(교정, 물리치료, 보톡스 등)에 관한 내용을 서술합니다.  
            예) 과거 물리치료, 보톡스 시술 이력  
            없으면 빈 문자열("").

            2. clinic_history_desc: 턱관절 장애와 관련된 과거 병력, 치료 이력, 발병 시기 및 계기를 서술합니다.  
            예) 3개월 전 증상 시작, 과거 외상 이력  
            없으면 빈 문자열("").

            3. factor_habbit: 환자의 음식 섭취 습관, 수면 자세, 이%%s 악물기 등 생활 습관에 관한 내용을 서술합니다.  
            예) 딱딱한 음식 섭취, 수면 중 치아 갈기  
            없으면 빈 문자열("").

            4. treat_plan: 현재 진행 중이거나 계획된 치료 방법을 서술합니다.  
            예) 물리치료 예정, 장치치료 권유  
            없으면 빈 문자열("").            
    ...
    def cc_analysis_prompt(texts):
        """CC 병력 및 습관 관련 프롬프트"""
        return f"""
        당신은 구강내과 전문의입니다. 아래는 환자의 상담 데이터의 주호소(CC) 텍스트입니다.
        텍스트에 없는 정보는 절대 추가하지 말고, 명확하게 언급된 정보만 사용하세요.
        누락된 내용은 빈 문자열("") 또는 null로 처리하세요.
        아래 각 항목은 텍스트에 명시된 내용을 기반으로 정확히 추출해야 합니다.
        ...
        1. location: 통증 또는 증상이 나타난 위치를 서술합니다.  
        예) 입가쪽, 왼쪽 턱, 양쪽 귀앞  
        텍스트에 언급되지 않았다면 빈 문자열("").

        2. pain_type: 통증 또는 증상의 종류를 서술합니다.  
        예) 뻐근함, 욱씬거림, 저림  
        언급되지 않으면 빈 문자열("").

        3. painUncomp_desc_jaw: 턱(악관절)과 관련된 통증이나 불편감을 설명합니다.  
        예) 턱 통증, 악관절에서 소리 남  
        해당 내용이 없으면 빈 문자열("").

        4. disable_desc_jaw: 턱 관절의 기능적 제한(예: 개구 제한, 좌우 움직임 제한)이나 비정상적인 소리 등 기능 장애를 설명합니다.  
        예) 제한된 개구, 턱 관절 움직임 제한  
        없으면 빈 문자열("").

        5. muscle_joint_desc_stress: 스트레스나 치아 갈기로 인한 턱 근육의 긴장이나 불편함을 서술합니다.  
        예) 이%%s 악물기, 근육 긴장  
        언급 없으면 빈 문자열("").
    ...
    def cc_severity_prompt(texts):
        """CC 심각도 관련 프롬프트"""
        return f"""
            당신은 구강내과 전문의입니다. 아래는 환자의 상담 데이터의 주호소(CC) 중 환자의 턱관절 통증 정도 정보입니다.
            텍스트에 없는 정보는 절대 추가하지 말고, 명확하게 언급된 정보만 사용하세요.
            누락된 내용은 빈 문자열("") 또는 null로 처리하세요.
            아래 각 항목은 텍스트에 명시된 내용을 기반으로 정확히 추출해야 합니다.
...
class TreatmentPrompts:
    ...
    def habit_prompt(texts):
        ...
        - 예를 들어, 치아 갈기, 치아 접촉 줄이기, 편측성 저작 등이 해당됩니다.
...
class PresentIllnessPrompts:
    ...
    def pi_occulusal_prompt(texts):
        ...
        return f"""
            ...
            - APS,SS Splint,APS Splint,이%%갈이장치,코골이장치
            아래 텍스트를 분석하여 위의 용어가 등장한다면 추출해주세요. 유사한 단어가 있다면 위 예시로 대체합니다.
            유사한 단어가 없다면 빈 문자열로 추출합니다.
            여러가지가 등장한다면 콤마로 구분합니다.
